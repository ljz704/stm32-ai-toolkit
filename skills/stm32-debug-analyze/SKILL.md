...
description: STM32 调试问题分析。当用户报告程序跑飞 死机 卡死 HardFault 进中断出不来 串口无输出或乱码 LED不亮 ADC采样不准 PWM无波形 上电无反应，或粘贴串口日志 报错 CFSR PC LR BFAR 寄存器值时使用。先收集结构化信息，读 .claude/memory/known_issues.md 历史教训，展示原始数据后给出根因与修复方案。
---
