...
function apply(ctx) {
	...
	// 并监视 mcp-servers.json 变化，文件被外部改写（install.py --dsh / 手动复制）时
	// 自动 diff 同步挂载/卸载，无需重启 DSH、无需手动调 API。
	let mcpWatcher = null;
	let mcpSyncTimer = null;
	let mcpSyncRunning = false;
	let mcpLastSignature = "";
	ctx.effect(() => {
		(async () => {
			await loadMCPState();
			mcpLastSignature = JSON.stringify(mcpServers);
			for (const server of mcpServers) {
				if (server.enabled !== false) await mountMCPServer(server);
			}
			// 文件监视：外部改写 mcp-servers.json 时自动同步（防抖 300ms + 签名去重）
			try {
				mcpWatcher = watch(MCP_STATE_FILE, () => {
					if (mcpSyncTimer !== null) clearTimeout(mcpSyncTimer);
					mcpSyncTimer = setTimeout(async () => {
						mcpSyncTimer = null;
						if (mcpSyncRunning) return;
						mcpSyncRunning = true;
						try {
							await loadMCPState();
							const signature = JSON.stringify(mcpServers);
							if (signature === mcpLastSignature) return; // 内容未变（如自己 save 触发）
							mcpLastSignature = signature;
							// diff：卸载不再存在或已禁用的，挂载新增/启用的
							const want = new Map(mcpServers.map((s) => [s.id, s]));
							for (const id of [...mcpDisposers.keys()]) {
								const s = want.get(id);
								if (!s || s.enabled === false) await unmountMCP(id);
							}
							for (const [id, s] of want) {
								if (s.enabled !== false && !mcpDisposers.has(id)) await mountMCPServer(s);
							}
							console.log(`[dsh-host-files] mcp-servers.json 变化已同步（${want.size} 个 server）`);
						} catch (error) {
							console.error("[dsh-host-files] MCP 文件同步失败:", error instanceof Error ? error.message : String(error));
						} finally {
							mcpSyncRunning = false;
						}
					}, 300);
				});
			} catch (error) {
				console.warn("[dsh-host-files] 无法监视 mcp-servers.json（外部改写需手动添加）:", error instanceof Error ? error.message : String(error));
			}
		})();
		return () => {
			if (mcpWatcher !== null) {
				try { mcpWatcher.close(); } catch {}
				mcpWatcher = null;
			}
			if (mcpSyncTimer !== null) {
